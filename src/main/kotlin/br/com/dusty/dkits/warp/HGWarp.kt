...
//	                          arrayOf(potions(1, false, true, STRENGTH, false), 1, 0.05, 1),
	...
	val preparation = 60 * 3
	val maxDuration = 60 * 20
	...
	fun end(gamers: Collection<Gamer>) {
		...
		val winners = arrayListOf<Gamer>()

		var highestKills = 0

		gamers.forEach {
			val warpKills = it.warpKills

			if (warpKills > highestKills) {
				winners.clear()
				winners.add(it)

				highestKills = warpKills
			} else if (warpKills == highestKills) {
				winners.add(it)
			}
		}
